...
rasterOptions(tmpdir="/Volumes/Macintosh HD/tmp/Rtmp/", tmptime = 24,progress="text",timer=TRUE,overwrite = T,chunksize=2e+06,maxmemory=1e+09)
```
...
#writeRaster(kendall_raster, "Processed/Temp_Kendall_US.tif")
...
Hist_LC_class<-reclassify(Historical_LC, c(-Inf, 7, NA,  8, 10, 1,  11, 14, 0, 15,Inf,NA))
...
Dif_LC <- Later_LC_class - Historical_LC_class
write.raster(Dif_LC, "Processed/Change_LC_FORESCE.tif")

plot(Dif_LC)
hist(Dif_LC)
rat <- levels(Dif_LC[[1]])
rat$landcover <- c('Reforestation', 'Deforestation', 'No Change')
rat$class <- c()
levelplot(Dif_LC, col.regions="green2red")
